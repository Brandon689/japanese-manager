...
export default function AdvancedDataViewComponent() {
    ...
    const listItem = (anime: Media, index: number) => {
        ...
        return (
            <div className={`col-12 ${anime === selectedItem ? 'selected' : ''}`} key={anime.id}>
                ...
                <Link to={id} state={anime}>
                    <div onClick={() => handleItemClick(anime)}
                         className={classNames('flex flex-column xl:flex-row xl:align-items-start p-4 gap-4', {'border-top-1 surface-border': index !== 0})}>
                        <img className="list-img shadow-2 block xl:block mx-auto" src={anime.coverImage?.large}
    ...
    const listTemplate = (animes: Media[], layout: string) => {
        if (layout === 'list') return <div
            className="">{animes.map((anime, index) => itemTemplate(anime, layout, index))}</div>;
        if (layout === 'grid') return <div
            className="data-grid">{animes.map((anime, index) => itemTemplate(anime, layout, index))}</div>;
    };
